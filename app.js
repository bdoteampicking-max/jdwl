...
function renderSchedule(date, data) {
  ...
  [1, 2, 3].forEach(shiftNo => {
    ...
    members.forEach(e => {
      ...
      row.innerHTML = `
        <div>
          <span class="emp-name">${e.nama}</span>
...
function loadAdminTable() {
  apiGet('employees').then(res => {
    ...
    if (res.data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Belum ada karyawan</td></tr>`;
      return;
    }
...
document.getElementById('btn-add-employee').addEventListener('click', () => {
  ...
  apiPost({ action: 'addEmployee', idToken: adminToken, employee }).then(res => {
    if (res.ok) {
      ...
      ['f-nama', 'f-jabatan', 'f-posisi', 'f-backup'].forEach(id => document.getElementById(id).value = '');
